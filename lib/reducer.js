...
var couchCache = require('./couchCacher').couchCache
...
function reducer(){
    ...
    function process_collated_record(tkey,fkey){
        return function(record){
            ...
            // big hack to test correcting the length.  some segment
            // lengths are excessive, upwards of 300 miles, etc.  The
            // vast majority are below 10 miles, and 10 miles is
            // reasonable to expect relatively constant traffic.

            // anyway, the value is parameterized via the environment
            // variable FREEWAY_TRUNCATE

            if(record.len > FREEWAY_TRUNCATE){
    ...
    function stash_out(featurehash){
        // must merge wim and vds here when and if I split them above


            featurehash.header = _.flatten(['ts','freeway','n','hh','not_hh'
